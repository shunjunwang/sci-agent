...
const registerSchema = z.object({ username: z.string().min(2, '用户名至少2个字符'), email: z.string().email('请输入有效邮箱'), password: z.string().min(6, '密码至少6位'), confirmPassword: z.string() }).refine(d => d.password === d.confirmPassword, { message: '两次密码不一致', path: ['confirmPassword'] });
...
export default function LoginPage() {
  ...
  const rf = useForm<z.infer<typeof registerSchema>>({ resolver: zodResolver(registerSchema), defaultValues: { username: '', email: '', password: '', confirmPassword: '' } });
  ...
  const handleRegister = async (data: z.infer<typeof registerSchema>) => { setError(''); setLoading(true); try { await register(data.username, data.email, data.password); router.push('/'); } catch (e: any) { setError(e.message); } finally { setLoading(false); } };
          ...
          <Tabs defaultValue="login">
            ...
            <TabsContent value="register">
              <form onSubmit={rf.handleSubmit(handleRegister)} className="space-y-4 mt-4">
                <div><Label htmlFor="ru">用户名</Label><Input id="ru" placeholder="你的用户名" {...rf.register('username')} />{rf.formState.errors.username && <p className="text-sm text-red-500 mt-1">{rf.formState.errors.username.message}</p>}</div>
